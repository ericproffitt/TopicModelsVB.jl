...
# 12 x 8
df = data.frame(x=1:2, Seconds=c(180, 2700), Processor=c('Intel Iris Plus Graphics 640', '2.3 GHz Dual-Core Intel Core i5'))
p = ggplot(df, aes(x=x, y=Seconds, fill=Processor)) +
    ...
    theme(axis.title.x=element_text(color='#424242', size=10), axis.line.x=element_line(color='grey'), axis.text.x=element_blank(), axis.ticks.x=element_blank()) +
    ...
    xlab("National Science Foundation Abstracts : Latent Dirichlet allocation model with 16 topics.") +
