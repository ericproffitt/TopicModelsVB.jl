...
**Important:** OpenCL, being open-source software, is not without its quirks, bugs, and rough edges.  If for some reason some of your model data is not read back into the CPU when checking the ELBO, and this results in a domain error, then your best bet is to set `chkelbo > iter` and retrain your model.  In addition, OpenCL does not allow for empty buffers, so empty corpora will cause the GPU models to return an error.  Finally, expect your computer to lag when training on your GPU, since you're effectively siphoning off its rendering resources to fit your model.
