type memCTM <: TopicModel
	...
	function memCTM(corp::Corpus, K::Integer)
		@assert ispositive(K)
		@assert !isempty(corp)
		checkcorp(corp)
		...
		model = new(K, M, V, N, C, copy(corp), topics, mu, sigma, invsigma, beta, betamem, lambda, vsq, lzeta, phi, 0, 0)
		for d in 1:M
...
function Elogpeta(model::memCTM, d::Int)
...
function updateMu!(model::memCTM)
	model.mu = sum(model.lambda) / model.M
end

function updateSigma!(model::memCTM)
...
function updateBeta!(model::memCTM)
	model.beta = model.betamem ./ sum(model.betamem, 2)
	model.betamem = zeros(model.K, model.V)
end

function updateBetaMEM!(model::memCTM, d::Int)
	terms, counts = model.corp[d].terms, model.corp[d].counts
	model.betamem[:,terms] += model.phi .* counts'
end

function updateLambda!(model::memCTM, d::Int, niter::Integer, ntol::Real)
	"Newton's method."
...
function updateVsq!(model::memCTM, d::Int, niter::Integer, ntol::Real)
	"Newton's method."
...
function updateLzeta!(model::memCTM, d::Int)
	model.lzeta = logsumexp(model.lambda[d] + 0.5 * model.vsq[d])	
end

function updatePhi!(model::memCTM, d::Int)
	terms = model.corp[d].terms
	model.phi = addlogistic(log(model.beta[:,terms]) .+ model.lambda[d], 1)
end

function train!(model::memCTM; iter::Integer=150, tol::Real=1.0, niter::Integer=1000, ntol::Real=1/model.K^2, viter::Integer=10, vtol::Real=1/model.K^2, chkelbo::Integer=1)
